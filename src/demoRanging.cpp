...
void RangingDemoInitApplication( uint8_t demoEntity )
{
    ...
    DemoSettings.RadioPowerMode = USE_LDO;
...
RangingDemoStatus_t RangingDemoRun( void )
{
    ...
    if ( DemoSettings.Entity == MASTER ) {
        ...
        // Serial.printf("RangingDemoRun : %u\n", demoInternalState);
        ...
        case APP_RANGING_CONFIG:
            if ( demoReady == true ) {
                ...
                Radio.SendPayload( demoBuffer, packetParams.Params.LoRa.PayloadLength, ( TickTime_t ) {
                    RX_TIMEOUT_TICK_SIZE, RNG_COM_TIMEOUT
                });
        ...
        case APP_RNG:
            if ( SendNext == true ) {
                ...
                // Serial.printf("measuredChannels: %u  DemoSettings.RngAntenna :%u\n", measuredChannels, DemoSettings.RngAntenna );
        ...
        case APP_RANGING_DONE: {
            ...
            Serial.printf("APP_RANGING_DONE: DemoResults.RngResultIndex:%u\n", DemoResults.RngResultIndex);
        }
        break;
        ...
        case APP_RANGING_CONFIG:
            ...
            Radio.SetRx( ( TickTime_t ) {
                RADIO_TICK_SIZE_1000_US, 0x1000
            } ); //0xFFFF
            demoInternalState = APP_IDLE;
            break;
        ...
        case APP_RNG:
            if ( SendNext == true ) {
                ...
                if ( measuredChannels <= DemoSettings.RngRequestCount ) {
                    ...
                    demoInternalState = APP_IDLE;
                    Radio.SetRx( ( TickTime_t ) {
                        RADIO_TICK_SIZE_1000_US, DemoSettings.RngReqDelay
                    } );
                } else {
                    TimerCancelTimer( );
                    demoStatus = DEMO_RANGING_TERMINATED;
        ...
        case APP_RX:
            if ( DemoSettings.RngStatus == RNG_INIT ) {
                ...
                if ( ( demoBufferSize > 0 ) && \
                    ...
                    Radio.SendPayload( demoBuffer, 9, ( TickTime_t ) {
                        RADIO_TICK_SIZE_1000_US, RNG_COM_TIMEOUT
                    } );
                    demoInternalState = APP_IDLE;
                } else {
                    demoInternalState = APP_RANGING_CONFIG;
...
uint8_t RangingDemoCheckDistance( void )
{
    ...
    if ( DemoResults.RngResultIndex > 0 ) {
        ...
        Serial.printf(">>>>>>>>>RangingDemoCheckDistance  __ displayRange:%.2f\n", displayRange);

        if ( j < DEMO_RNG_CHANNELS_COUNT_MIN ) {
            DemoSettings.RngStatus = RNG_PER_ERROR;
            Serial.printf("RNG_PER_ERROR! : %d\n", j);
        } else {
            DemoSettings.RngStatus = RNG_VALID;
            Serial.println("RNG_VALID!");
        }
        ...
        if ( displayRange < 0 ) {
            ...
            case DEMO_RNG_UNIT_SEL_M:
                ...
                Serial.printf("DEMO_RNG_UNIT_SEL_M: %.2f\n", displayRange);
                break;

            case DEMO_RNG_UNIT_SEL_YD:
                DemoResults.RngDistance = displayRange * DEMO_RNG_UNIT_CONV_YD;
                Serial.printf("DEMO_RNG_UNIT_SEL_YD: %.2f\n", DemoResults.RngDistance);
                break;
    ...
    // Serial.printf("RangingDemoCheckDistance......................Done:%u !\n", j);
    return j;
}
...
void OnRxError( IrqErrorCode_t errorCode )
{
    ...
    Serial.printf("OnRxError code : %u\n", errorCode);
}
